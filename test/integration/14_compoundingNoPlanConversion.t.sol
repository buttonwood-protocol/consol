...
/**
 * @title Integration_14_CompoundingNoPlanConversionTest
 * @author @SocksNFlops
 * @notice Borrower makes a compounding mortgage without a payment plan. Gets the entire mortgage converted.
 */
contract Integration_14_CompoundingNoPlanConversionTest is IntegrationBaseTest {
  using MortgageMath for MortgagePosition;

  function integrationTestId() public pure override returns (string memory) {
    return type(Integration_14_CompoundingNoPlanConversionTest).name;
  }
  ...
  function run() public virtual override {
    ...
    vm.startPrank(borrower);
    loanManager.redeemMortgage(mortgagePosition.tokenId, true);
    vm.stopPrank();

    // Validate that the mortgage has been redeemed
    mortgagePosition = loanManager.getMortgagePosition(1);
