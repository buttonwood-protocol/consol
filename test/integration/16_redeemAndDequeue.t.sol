...
/**
 * @title Integration_16_RedeemAndDequeueTest
 * @author @SocksNFlops
 * @notice Redeem a mortgage and then dequeue it off the ConversionQueue.
 */
contract Integration_16_RedeemAndDequeueTest is IntegrationBaseTest {
  using MortgageMath for MortgagePosition;

  function integrationTestId() public pure override returns (string memory) {
    return type(Integration_16_RedeemAndDequeueTest).name;
  }
  ...
  function run() public virtual override {
    ...
    // Borrower pays off the entire mortgage
    ...
    vm.startPrank(borrower);
    loanManager.redeemMortgage(mortgagePosition.tokenId, false);
    vm.stopPrank();

    // Validate that the mortgage position has the status REDEEMED
    mortgagePosition = loanManager.getMortgagePosition(mortgagePosition.tokenId);
    assertEq(uint8(mortgagePosition.status), uint8(MortgageStatus.REDEEMED), "status");
