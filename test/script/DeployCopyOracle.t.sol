...
import {DeployCopyOracle} from "../../script/DeployCopyOracle.s.sol";
...
contract DeployCopyOracleTest is Test {
  ...
  function setUp() public {
    ...
    admin = makeAddr("Copy oracle admin");
    signer = makeAddr("Copy oracle signer");
    ...
    vm.setEnv("COPY_ORACLE_ADMIN", vm.toString(admin));
    vm.setEnv("COPY_ORACLE_SIGNER", vm.toString(signer));
  ...
  function test_run() public {
    ...
    DeployCopyOracle deploy = _prepare("DeployCopyOracleTest", collaterals, address(generalManager));
    deploy.run();

    // The store is configured from the environment
    CopyOracle copyOracle = deploy.copyOracle();
    assertEq(copyOracle.signer(), signer, "Signer mismatch");
    assertTrue(copyOracle.hasRole(copyOracle.DEFAULT_ADMIN_ROLE(), admin), "Admin role missing");
    assertFalse(copyOracle.hasRole(copyOracle.DEFAULT_ADMIN_ROLE(), deployerAddress), "Deployer must not be admin");
    ...
    for (uint256 i = 0; i < collaterals.length; i++) {
      CopyOraclePriceOracle priceOracle = deploy.priceOracles(i);
      assertEq(address(priceOracle.copyOracle()), address(copyOracle), "Store mismatch");
    ...
    // The address book gains copyOracleAddress, replaces priceOracles in order, and keeps every other key
    string memory json = vm.readFile(deploy.getPath());
    assertEq(vm.parseJsonAddress(json, ".copyOracleAddress"), address(copyOracle), "copyOracleAddress mismatch");
  ...
  function test_run_collateralLengthMismatch() public {
    ...
    DeployCopyOracle deploy = _prepare("DeployCopyOracleTestLength", collaterals, address(generalManager));
  ...
  function test_run_deployerNotAdmin() public {
    ...
    DeployCopyOracle deploy = _prepare("DeployCopyOracleTestAdmin", collaterals, address(foreignManager));
  ...
  function test_run_collateralDecimalsMismatch() public {
    ...
    DeployCopyOracle deploy = _prepare("DeployCopyOracleTestDecimals", collaterals, address(generalManager));
  ...
  function _prepare(string memory suffix, address[] memory collaterals, address generalManagerAddress)
    internal
    returns (DeployCopyOracle deploy)
  {
    deploy = new DeployCopyOracle();
